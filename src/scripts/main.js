const { PerfMeter } = require('./perfomance');
const { generate5MinsOHLCData } = require('./data-Generator');
...
var chartType = $('#anystock-speed-test-chartType-select')
...
function changeChart() {
  ...
  if (type == 'configurationType') {
    ...
    createChart(
      initialPointsCount,
      chartConfiguration,
      getFunctionForCreateChart(chartType)
    );
...
function execCreateStock(pointsCount, chartConfiguration) {
  rawData = generate5MinsOHLCData(
    new Date(new Date().getUTCFullYear() - 4, 0),
    pointsCount,
    100,
    10,
    100
  ...
  resultContainer.append(
    '<h5>Rendering ' + addCommas(pointsCount) + ' Data Points</h5>'
  );
  resultContainer.append(
    generateHTMLStatRecord(
      'Creating data storage',
      perfMeter.get('Creating data storage')
    )
  );
  resultContainer.append(
    generateHTMLStatRecord(
      'Creating chart instance',
      perfMeter.get('Creating chart instance')
    )
  );
  resultContainer.append(
    generateHTMLStatRecord('Chart rendering', perfMeter.get('Chart rendering'))
  );
  resultContainer.append(
    generateHTMLStatRecord('Total', perfMeter.get('Total'))
  );
}
...
function startStreaming() {
  ...
  streamingTimerId = setInterval(function () {
    ...
    function streamData() {
      ...
      if (isNaN(streamingAverage))
        streamingAverage = perfMeter.get(streamPointsCount);

      var streamingStat = $('#anystock-speed-test-stream-stat').length;
      if (!streamingStat) {
        var resultContainer = $('#anystock-speed-test-base-result');
        resultContainer.append(
          '<h5 id="anystock-speed-test-stream-stat">Streaming ' +
            addCommas(streamPointsCount) +
            ' Data Points</h5>'
        );
        resultContainer.append(
          generateHTMLStatRecord(
            ...
            streamingInterval,
            undefined,
            'anystock-speed-test-stream-interval'
          )
        );
        resultContainer.append(
          generateHTMLStatRecord(
            'Average rendering time',
            Math.round(streamingAverage),
            undefined,
            'anystock-speed-test-stream-average'
          )
        );
      } else {
        streamingAverage =
          (streamingAverage + perfMeter.get(streamPointsCount)) / 2;
        $(
          '#anystock-speed-test-stream-average .anystock-speed-test-result-row-value'
        ).html(Math.round(streamingAverage) + 'ms');
      }
    }
  }, streamingInterval);
}
...
function showPreloader() {
  $('#anystock-speed-test-base-chart').append(
    '<div id="loader-wrapper-chart" class="anychart-loader"><div class="rotating-cover"><div class="rotating-plane"><div class="chart-row"><span class="chart-col green"></span><span class="chart-col orange"></span><span class="chart-col red"></span></div></div></div></div>'
  );
}
...
function generateHTMLStatRecord(title, value, postfix, opt_id) {
  ...
  return (
    '<div ' +
    opt_id +
    'class="anystock-speed-test-result-row clearfix"> ' +
    '<span class="anystock-speed-test-result-row-title">' +
    title +
    ': </span>' +
    '<span class="anystock-speed-test-result-row-value">' +
    Math.round(value) +
    postfix +
    '</span>' +
    '</div>'
